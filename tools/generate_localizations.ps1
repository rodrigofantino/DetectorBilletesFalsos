...
function Get-TranslationMap {
	...
	while ($pending.Count -gt 0) {
		...
		$uri = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=$Locale&dt=t&q=$([uri]::EscapeDataString($source))"
...
function Add-MissingCatalogTexts {
	param($Catalog, [string]$Locale, [hashtable]$Translations)
...
foreach ($locale in $targetLocales) {
	...
	Add-MissingCatalogTexts $catalog $locale $translations
